...
- **MCG EXL3 only.** The served MiMo target is the K2.5 MCG EXL3 pack
  (`MiMo-V2.6-Flash-Sushi2.5bpw`). The affine pack
...
- **A hot entry keeps a prompt-end ring checkpoint** (window + 30 rows per sliding layer): a reply past ~256 tokens
  ...
  ring (158 rows: 30 MiB bf16, 16 MiB kv8); each entry bills its own in `kv_bytes`. Live (2026-09-24, kv8, default
  cache, `taskpolicy -a`, main 298165c vs the landing commit): a 2000-word essay, a 600-word follow-up, then a
  question: turn 3 TTFT 3.38 -> 1.34 s (cold -> restored 2525 of 3430).
