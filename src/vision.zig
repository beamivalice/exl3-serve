...
pub const VisionEncoder = struct {
    ...
    /// Build a VisionEncoder wrapping the MiMo-ViT. Same sentinel shape as
    /// `initQwen`; `mimo` drives `forwardPatches`.
    fn initMimo(allocator: std.mem.Allocator, config: ModelConfig, weights: *const Weights) !VisionEncoder {
        const s = mlx.mlx_default_gpu_stream_new();
    ...
    pub fn forward(self: *VisionEncoder, pixels: mlx.mlx_array) !mlx.mlx_array {
        ...
        if (self.qwen != null or self.muse != null or self.lfm2 != null or self.mimo != null) return error.PixelsMissingPatchGrid;
