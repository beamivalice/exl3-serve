...
BINARY="${SUSHI_BINARY:-./zig-out/bin/sushi}"; LABEL="${BENCH_LABEL:-$(basename $(dirname $(dirname $BINARY)))}"
LOG=$(mktemp)
SUSHI_ROUND_COST_PERSIST=0 "$BINARY" --model "$MODEL" --serve --port "$PORT" --no-pld --mtp --max-concurrent 16 --prefix-cache-entries 0 > "$LOG" 2>&1 &
...
req() { # prompt mtp -> "tokens wall_s"
    ...
    local n=$(python3 -c "import json,sys; print(json.dumps({'model':'sushi','messages':[{'role':'user','content':sys.argv[1]}],'max_tokens':$MAXTOK,'temperature':0,'enable_mtp':sys.argv[2]=='true','enable_drafter':sys.argv[2]=='true'}))" "$1" "$2" |
