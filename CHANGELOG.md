...
- **`/cd <folder>` in `sushi run`**: moves the folder the file tools and `/image` read from, and a model that asks for
  a file outside it now suggests `/cd` instead of guessing other paths.
