...
- **Tool-call arguments stream as they are written**: a long file write no longer arrives in one piece at the end,
  so agents with a stream idle timeout (omp, pi) stop aborting and retrying it.
